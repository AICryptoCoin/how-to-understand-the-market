# Z29 · Отчёт: Цена подглядывания (Круг 3)
...
* **Базовый коммит:** `176011d522e9b4ee9333d0fe69cafefeec2786aa` (HEAD ветки при выдаче). База круга 2 — `21927cf`. База круга 3 — `e2e156f`.
...
## 2. Таблица пяти утечек на приборе Z28 (Круг 3, Р11)
...
Согласно решению Р3 и Р11, в таблице представлены три блока оценок в едином смысле для всех пяти утечек:
1. **Поддельный выигрыш $d$** (`fake_gain_metrics`): средняя [ДИ], $t_{\mathrm{honest}}$, вердикт. Сравнение исследователя «мой прогноз с утечкой против честного базового».
2. **Сравнение `Z28` при утечке** (`z28_shift_metrics`): средняя [ДИ], $t_{\mathrm{honest}}$, вердикт. Для базовых утечек (L1, L2, L4) — сдвиг $\delta - d$ (сравнение честной цены с базовым-с-утечкой); для ценовых утечек (L3, L5) — $d$ (сравнение цены с утечкой с честным базовым).
3. **Прирост к честному `Z28` ($d - \delta$)** (`fake_gain_over_honest_price`): только для ценовых утечек L3 и L5 (у базовых утечек — «—»).

### Сводная таблица результатов

| Код | Утечка и описание | Аудит | Первая дата | Дословное сообщение аудита | Поддельный выигрыш $d$ [95 % ДИ], п.п. | $t_{\mathrm{honest}}$ ($d$) | Вердикт $d$ | Сравнение Z28 при утечке [95 % ДИ], п.п. | $t_{\mathrm{honest}}$ (Z28) | Вердикт Z28 при утечке | Прирост к честному Z28 ($d - \delta$) [95 % ДИ], п.п. |
|---|---|:---:|:---:|---|---|:---:|:---:|---|:---:|:---:|:---:|
...
| **L3** (Yahoo)<br>**L3** (Shiller) | Опорная волатильность по всей выборке (константа) | **НЕ ПОЙМАН**<br>**НЕ ПОЙМАН** | —<br>— | `OK` (пропущена)<br>`OK` (пропущена) | $-0{,}3061$ $[-1{,}0771, +0{,}4649]$<br>$-0{,}1049$ $[-0{,}4287, +0{,}2190]$ | $-0{,}78$<br>$-0{,}63$ | `information_absent`<br>`information_absent` | $-0{,}3061$ $[-1{,}0771, +0{,}4649]$<br>$-0{,}1049$ $[-0{,}4287, +0{,}2190]$ | $-0{,}78$<br>$-0{,}63$ | `information_absent`<br>`information_absent` | $-0{,}4749$ $[-1{,}1384, +0{,}1886]$<br>$-0{,}0483$ $[-0{,}0986, +0{,}0020]$ |
...
### Механика ловушки базового соперника (Р3, Р13)
При утечке в базовом сопернике (L2: включение незавершённых годовых окон) базовый ансамбль получает искусственное преимущество над реальностью ($+0{,}19$ п.п. на Yahoo и $+0{,}13$ п.п. на Шиллере). В результате сравнение честной цены с базовым-с-утечкой ($\delta - d$) делает честный прогноз **хуже, чем он есть на самом деле**: относительный выигрыш занижается с $+0{,}17$ п.п. до $-0{,}02$ п.п. на Yahoo и с $-0{,}06$ п.п. до $-0{,}18$ п.п. на Шиллере (при этом сам честный прогноз по цене в `Z28` информации не несёт — вердикт `information_absent`, интервал включает ноль). Это формирует зеркальную ловушку: исследователь видит честную модель в заведомо проигрышном свете, потому что сравнивает её с соперником, неявно заглядывающим в будущее.
...
Винтаж 1965Q4 в варианте (B) является первым винтажом набора данных, в котором впервые «появляются» все более ранние кварталы истории, что является артефактом старта датасета, а не публикацией в реальном времени; это событие лежит вне окна счёта ($t < 1982\text{-}01$) и на итоговый счёт влияния не оказывает.
...
* **Утверждение A (`NAHB` $\to$ `UNRATE`):**
  ...
  * Результат идентичен на обоих маршрутах (ALFRED и FRED API).
* **Утверждение B (`NAHB YoY` $\to$ `US10Y YoY`):**
  * Сдвиг значений B вызван не `NAHB HMI` (утверждения A и B строят $x$ одинаково — `log_change(hmi, 12)`, и A на том же $x$ воспроизводится точно; значит, `NAHB` на этих датах не менялся), а рядом `DGS10` маршрута ALFRED (`alfredgraph.csv`) против FRED API.
  * **Маршрут ALFRED (без ключа):**
    * Внутри выборки: пик на лаге 17, $r = +0{,}3046$ ($n = 360$).
    * Вне выборки: $r$ на внутреннем пике $+0{,}0668$, пик вне выборки: лаг 20, $r = +0{,}0947$ ($n = 127$).
  * **Маршрут FRED API с ключом (замер архитектора 2026-09-25, главное дерево, функции `Z01`):**
    * Внутри выборки: пик на лаге 17, $r = +0{,}2981$ — **точное совпадение** с `Z01/result.json`.
    * Вне выборки: $r$ на внутреннем пике $+0{,}0673$, пик вне выборки: лаг 20, $r = +0{,}0962$ ($n = 129$: ряд стал на два месяца длиннее).

### 2. Чистый разрез (внутри выборки $t \le 2009\text{-}12$, вне выборки $t \ge 2013\text{-}01$)

| Утверждение и маршрут | Внутри выборки: пик (лаг, $r$) | Вне выборки: $r$ на внутреннем пике | Вне выборки: пик (лаг, $r$) | Статус |
|---|:---:|:---:|:---:|---|
| **A (`NAHB` $\to$ `UNRATE`)** [ALFRED и FRED API] | **лаг 17**, $r = -0{,}6144$ ($n = 288$) | $r = -0{,}3141$ | **лаг 0**, $r = -0{,}3827$ ($n = 117$) | Лид смещается с 19 на 17 мес., вне выборки уезжает в синхронность (лаг 0) |
| **B (`NAHB YoY` $\to$ `US10Y YoY`)** [ALFRED, без ключа] | **лаг 34**, $r = +0{,}2355$ ($n = 288$) | $r = -0{,}0195$ | **лаг 20**, $r = +0{,}0947$ ($n = 127$) | Лид 18 мес. полностью распадается: пик уходит на край окна (лаг 34), связь вне выборки нулевая |
| **B (`NAHB YoY` $\to$ `US10Y YoY`)** [FRED API, замер архитектора 2026-09-25] | **лаг 35**, $r = +0{,}2360$ | $r = +0{,}0041$ (на лаге 35) | **лаг 20**, $r = +0{,}0962$ ($n = 129$) | Лид 18 мес. полностью распадается: пик уходит на край окна (лаг 35), связь вне выборки нулевая |

*Адрес в `result.json`: `z01_audit` (поле `status: recalculated_via_alfred`).*

Вывод по утверждению B на обоих маршрутах абсолютно идентичен: внутренний пик смещается к правому краю окна лагов (34–35 месяцев), а вне выборки связь на этом пике падает практически до нуля ($-0{,}0195$ на ALFRED и $+0{,}0041$ на FRED API).
...
## 8. Протокол сдачи и ворота круга 3
