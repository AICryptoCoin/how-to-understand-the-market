...
4. баланс фигурных скобок внутри формулы — проходом с пропуском
   экранированного знака, чтобы скобки множества `\\{…\\}` не путали счёт.
   Добавлен 2026-09-24: класс нашёл исполнитель сверх брифа.
...
def _brace_balance(body):
    """Открытые и закрытые группы {…} и признак, что закрытие шло раньше открытия.

    Проход по знакам, а не счёт: знак после обратного слэша экранирован и в
    группы не входит. Иначе \\{ и \\} множества {−4,25; 3,25} из главы 10 давали
    бы ложную тревогу, а \\\\ (перевод строки) съедал бы следующую скобку.
    """
...
def check(raw: str) -> list:
    ...
    # 4. Баланс фигурных скобок - тоже внутри каждой формулы. Непарная скобка
    #    роняет разбор MathJax, и формула уходит к читателю красной строкой.
    #    Класса не было до 2026-09-24: его нашёл исполнитель в круге правок
    #    главы 10, проверяя "что ещё могло сломаться" сверх брифа.
    for m in REGION.finditer(raw):
        body = next(g for g in m.groups() if g is not None)
        opened, closed, negative = _brace_balance(body)
        if opened != closed or negative:
            why = ("закрывающая раньше открывающей" if negative and opened == closed
                   else "%d против %d" % (opened, closed))
            out.append("непарные фигурные скобки внутри формулы: %s, строка %d"
                       % (why, _line_of(raw, m.start())))
